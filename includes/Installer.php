...
class Installer {
    ...
    public function timestamps() {
        ...
        $installed = get_option( 'weidocs_installed' );

        if ( ! $installed ) {
            update_option( 'weidocs_installed', time() );
        }

        update_option( 'weidocs_version', WEDOCS_VERSION );
    }
    ...
    public function maybe_create_docs_page() {
        $version = get_option( 'weidocs_version' );
        ...
        // skip if there's a page already with [weidocs] shortcode
        ...
        if ( ! is_wp_error( $docs_page ) ) {
            $settings              = get_option( 'weidocs_settings', [] );
            $settings['docs_home'] = $docs_page;

            update_option( 'weidocs_settings', $settings );
        }
    }
}
