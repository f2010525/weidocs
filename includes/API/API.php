...
class API extends WP_REST_Controller {
            ...
            [
                ...
                'args'                => [
                    'query' => [
                        'required'          => true,
                        'type'              => 'string',
                        'description'       => __( 'Limit results to those matching a string.', 'weidocs' ),
                        'sanitize_callback' => 'sanitize_text_field',
                    ...
                    'in' => [
                        'required'          => false,
                        'type'              => 'integer',
                        'description'       => __( 'The ID for the parent of the object.', 'weidocs' ),
                        'sanitize_callback' => 'absint',
                        'validate_callback' => 'rest_validate_request_arg',
                    ],
                    'page' => [
                        'description'       => __( 'Current page of the collection.', 'weidocs' ),
                        'type'              => 'integer',
                        'default'           => 1,
                        'sanitize_callback' => 'absint',
                        'validate_callback' => 'rest_validate_request_arg',
                    ...
                    'per_page' => [
                        'description'       => __( 'Maximum number of items to be returned in result set.', 'weidocs' ),
                        'type'              => 'integer',
                        'default'           => 10,
                        'minimum'           => 1,
                        'maximum'           => 100,
    ...
    public function handle_feedback( $request ) {
        ...
        weidocs_doc_feedback_email( $id, $name, $email, $request['subject'], $request['message'] );
    ...
    public function search_docs( $request ) {
        ...
        if ( $request['in'] ) {
            ...
            $children_docs = weidocs_get_posts_children( $request['in'], 'docs' );
        ...
        if ( $page > $max_pages && $total_posts > 0 ) {
            return new WP_Error( 'rest_docs_invalid_page_number', __( 'The page number requested is larger than the number of pages available.', 'weidocs' ), [ 'status' => 400 ] );
        }
    ...
    public function create_item_permissions_check( $request ) {
        ...
        if ( empty( $request['name'] ) ) {
            return new WP_Error( 'rest_doc_invalid_name', __( 'No name given', 'weidocs' ) );
        }

        if ( empty( $request['email'] ) ) {
            return new WP_Error( 'rest_doc_invalid_email', __( 'No email given', 'weidocs' ) );
        }
