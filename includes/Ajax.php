...
class Ajax {
    ...
    public function __construct() {
        add_action( 'wp_ajax_weidocs_create_doc', [ $this, 'create_doc' ] );
        add_action( 'wp_ajax_weidocs_remove_doc', [ $this, 'remove_doc' ] );
        add_action( 'wp_ajax_weidocs_admin_get_docs', [ $this, 'get_docs' ] );
        add_action( 'wp_ajax_weidocs_sortable_docs', [ $this, 'sort_docs' ] );

        add_action( 'wp_ajax_weidocs_rated', [ $this, 'hide_weidocs_rating' ] );

        // feedback
        add_action( 'wp_ajax_weidocs_ajax_feedback', [ $this, 'handle_feedback' ] );
        add_action( 'wp_ajax_nopriv_weidocs_ajax_feedback', [ $this, 'handle_feedback' ] );

        // contact
        add_action( 'wp_ajax_weidocs_contact_feedback', [ $this, 'handle_contact' ] );
        add_action( 'wp_ajax_nopriv_weidocs_contact_feedback', [ $this, 'handle_contact' ] );
    }
    ...
    public function create_doc() {
        check_ajax_referer( 'weidocs-admin-nonce' );
    ...
    /**
     * Assume the user rated weiDocs.
     *
     * @return void
     */
    public function hide_weidocs_rating() {
        check_ajax_referer( 'weidocs-admin-nonce' );

        update_option( 'weidocs_admin_footer_text_rated', 'yes' );
        wp_send_json_success();
    }
    ...
    public function handle_feedback() {
        check_ajax_referer( 'weidocs-ajax' );

        $template = '<div class="weidocs-alert weidocs-alert-%s">%s</div>';
        $previous = isset( $_COOKIE['weidocs_response'] ) ? explode( ',', $_COOKIE['weidocs_response'] ) : [];
        $post_id  = intval( $_POST['post_id'] );
        $type     = in_array( $_POST['type'], [ 'positive', 'negative' ] ) ? $_POST['type'] : false;

        // check previous response
        if ( in_array( $post_id, $previous ) ) {
            $message = sprintf( $template, 'danger', __( 'Sorry, you\'ve already recorded your feedback!', 'weidocs' ) );
        ...
        if ( $type ) {
            ...
            $val = setcookie( 'weidocs_response', $cookie_val, time() + WEEK_IN_SECONDS, COOKIEPATH, COOKIE_DOMAIN );
        }

        $message = sprintf( $template, 'success', __( 'Thanks for your feedback!', 'weidocs' ) );
    ...
    public function handle_contact() {
        ...
        if ( !is_user_logged_in() ) {
            ...
            if ( !$email ) {
                wp_send_json_error( __( 'Please enter a valid email address.', 'weidocs' ) );
            }
        } else {
        ...
        if ( empty( $subject ) ) {
            wp_send_json_error( __( 'Please provide a subject line.', 'weidocs' ) );
        }

        if ( empty( $message ) ) {
            wp_send_json_error( __( 'Please provide the message details.', 'weidocs' ) );
        }

        weidocs_doc_feedback_email( $doc_id, $name, $email, $subject, $message );

        wp_send_json_success( __( 'Thanks for your feedback.', 'weidocs' ) );
    }
